...
namespace dEnc
{
    ...
    void AmmrClient<DPRF>::encrypt(span<block> ptxt, std::vector<block>& ctxt)
	{
		...
		// std::cout << typeid(this->mDprf).name() << "\n";
		...
		// eval DPRF(x)
		// std::string x(typeid(this->mDprf).name());
		// std::string dprftype = x.substr(9, x.length()-10);
  //       if(dprftype.compare("LWRSymDprf") == 0){
		// 	auto fx = mDprf->eval(alpha, 0);
		// }
		// else{
    ...
    AsyncEncrypt AmmrClient<DPRF>::asyncEncrypt(span<block> ptxt, std::vector<block>& ctxt)
	{
        ...
        // std::cout << p << "\n";
        block alpha;
        // std::cout << "2: " << typeid(this->mDprf).name() << "\n";
		// hash the {message, rho} to get the DPRF input
		oc::RandomOracle H(sizeof(block));
		H.Update((u8*)ptxt.data(), ptxt.size() * sizeof(block));
		H.Update(p);
		...
		// std::string dprftype = x.substr(9, x.length()-10);
  //       if(dprftype.compare("LWRSymDprf") == 0){
  //       	state->async = mDprf->asyncEval(alpha, 0);
  //       }
  //       else{
